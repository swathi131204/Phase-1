...
const App = () => {
  ...
  const updateUser = () => {
    ...
    axios
      .put(`https://jsonplaceholder.typicode.com/users/${updateId}`, { name })
      .then((res) => {
        setUsers(users.map((user) => (user.id === updateId ? { ...user, name } : user)));
        setUpdateId(null);
        setName("");
    ...
    <div>
      ...
      {users.map((u) => (
        <div key={u.id}>
          ...
          <button onClick={() => deleteUser(u.id)} style={{ marginLeft: "5px", color: "red" }}>
            Delete
          </button>
        </div>
      ))}
      ...
      />
      {updateId ? <button onClick={updateUser}>Update</button> : <button onClick={postUser}>Post</button>}
    </div>
  );
};
